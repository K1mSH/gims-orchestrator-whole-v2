            ...
            for (Resource resource : resources) {
                try (InputStream is = resource.getInputStream()) {
                    ...
                    if (def != null && def.getAgentCode() != null) {
                        ...
                        log.info("[Provide] Agent 설정 로드: {} (type={}, steps={})",
                                def.getAgentCode(), def.getType(), def.getSteps().size());
                    }
                } catch (Exception e) {
                    log.error("[Provide] Agent 설정 로드 실패: {}", resource.getFilename(), e);
                }
            }

            log.info("[Provide] 총 {} 개 Agent 설정 로드 완료", agentDefinitions.size());
        } catch (Exception e) {
            log.error("[Provide] Agent 설정 파일 스캔 실패", e);
        }
    }
