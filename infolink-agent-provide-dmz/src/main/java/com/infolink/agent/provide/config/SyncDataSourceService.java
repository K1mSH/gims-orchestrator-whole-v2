package com.infolink.agent.provide.config;
...
/**
 * Provide Agent - 파이프라인 실행 시 Source/Target DataSource 관리
 */
@Slf4j
@Service
public class SyncDataSourceService implements DataSourceProvider {
    ...
    public JdbcTemplate getJdbcTemplate(String datasourceId) {
        ...
        log.debug("[Provide] DataSource '{}' 미해석, 기본 JdbcTemplate (로컬 DB) 사용", datasourceId);
    ...
    private HikariDataSource createDataSource(String datasourceId) {
        log.info("[Provide] DataSource 생성: {}", datasourceId);
        ...
        hikariConfig.setPoolName("ProvidePool-" + datasourceId);
        ...
        log.info("[Provide] DataSource 생성 완료: {} -> {} (maxPool=10, timeout=10s, leak=60s)", datasourceId, info.getJdbcUrl());
        return ds;
    }
    ...
    public DataSourceInfo resolveFromProxy(String datasourceId) {
        ...
        if (cached != null) {
            log.debug("[Provide] DataSource 캐시에서 해석: {}", datasourceId);
            return cached;
        }

        if (proxyUrl == null || proxyUrl.isEmpty()) {
            throw new IllegalStateException("[Provide] proxy-url 미설정. 자격증명 해석 불가: " + datasourceId);
        }
        ...
        throw new IllegalStateException("[Provide] Proxy에서 datasource 해석 실패: " + datasourceId);
    }
    ...
    private DataSourceInfo fetchConnectionInfoFromProxy(String datasourceId) {
        try {
            ...
            log.info("[Provide] Proxy에서 데이터소스 정보 조회: {}", datasourceId);
            ...
            if (response == null || response.isEmpty()) {
                log.warn("[Provide] Proxy에서 빈 응답 수신 (datasource: {})", datasourceId);
                return null;
            }
            ...
            log.info("[Provide] Proxy에서 데이터소스 조회 완료: {} ({}:{})",
                    datasourceId, info.getHost(), info.getPort());
            return info;
        } catch (Exception e) {
            log.warn("[Provide] Proxy에서 데이터소스 조회 실패: {} - {}", datasourceId, e.getMessage());
            return null;
        }
    }
    ...
    public String checkPoolHealth(String datasourceId) {
        ...
        log.info("[Provide] Pool 상태 점검 '{}': active={}/{}, waiting={}, total={}",
    ...
    public void setCurrentDatasources(DataSourceInfo sourceDatasource, DataSourceInfo targetDatasource) {
        if (sourceDatasource != null) {
            ...
            log.info("[Provide] 현재 source 데이터소스 설정: {} ({}:{})",
        ...
        if (targetDatasource != null) {
            ...
            log.info("[Provide] 현재 target 데이터소스 설정: {} ({}:{})",
    ...
    public void clearCurrentDatasources() {
        ...
        log.debug("[Provide] 현재 데이터소스 정보 초기화");
    }
    ...
    public void closeAll() {
        dataSources.forEach((id, ds) -> {
            if (!ds.isClosed()) {
                ds.close();
                log.info("[Provide] DataSource 종료: {}", id);
            }
        });
        dataSources.clear();
        jdbcTemplates.clear();
