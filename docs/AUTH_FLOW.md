...
[ Backend 8080 ]
  ...
  ④ JwksClient 준비 (auth 모듈 URL = http://auth:9096/.well-known/jwks.json)
...
[ 첫 API 호출 들어옴 (예: 10분 후 운영자 진입) ]
   ...
   ② 캐시 lookup → 비어있음 ❌
   ③ JwksClient.fetch():
        - GET http://auth:9096/.well-known/jwks.json
...
```
[ 사용자 ]    [ Frontend 3000 ]        [ Auth 모듈 9096 ]
...
```
[ 사용자 bob ]                 [ Frontend /users/me ]              [ Auth 9096 ]
  현재 비번: passXYZ
...
[ 검증자 모듈 ]
  ...
  ② 캐시 만료 (NOW - cacheLoadedAt > 5min)
       ↓
  ③ JwksClient.fetch() 호출:
       ┌── 첫 시도: GET http://auth:9096/.well-known/jwks.json
...
- 검증자 모듈 = JWKS endpoint 호출하지만 **내부망의 auth 모듈** (`http://auth:9096`)
- **운영 폐쇄망 그대로 가능**
