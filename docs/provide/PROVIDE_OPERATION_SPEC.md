...
> 대상: `infolink-api-provider` 의 `api_prv_operation` / `api_prv_operation_column` / `api_prv_operation_param` 테이블
...
`infolink-api-provider` 의 DB 엔티티를 UI 가 그대로 노출하므로 **관리 UI 의 한글 라벨 = 아래 표의 "한글 라벨" 열**. 영문 필드명은 이 문서 본문(§5) 표에 나오는 이름. 둘이 같은 대상입니다.
...
- SQL 문제 디버그: `logging.level.com.infolink.provider: DEBUG` (이미 설정됨) — 실행된 SQL 이 로그에 찍힘
