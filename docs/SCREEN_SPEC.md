...
6. **"조건실행 ▾"** — 클릭하면 아래에 WHERE 조건 입력 패널이 펼쳐집니다.
7. **"삭제"** — Agent를 삭제합니다 (확인 다이얼로그).

## 8.2 조건실행 (WHERE 조건)

"조건실행 ▾" 버튼을 클릭하면 WHERE 조건 입력 패널이 펼쳐집니다.

### 8.2.1 WHERE 조건 입력하기

1. "조건 추가" 버튼으로 조건을 추가합니다.
2. 테이블/컬럼 드롭다운에서 대상을 선택합니다 (Agent YML의 select-tables 기반).
3. 연산자를 선택하고 값을 입력합니다 (dataType에 따라 입력 타입 자동 전환).
