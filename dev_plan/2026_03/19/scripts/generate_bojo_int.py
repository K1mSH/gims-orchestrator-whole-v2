# -*- coding: utf-8 -*-
"""
sync-agent-bojo-int NGIS D23 클래스명세서 생성 스크립트
- 2개 도메인 그룹, 5개 클래스
"""
...
def build_all_classes():
    ...
    health_classes.append(('Controller', [{
        ...
        'overview': make_overview(
            ...
            '내부망 Agent 상태 확인 및 디버그 정보 제공. 기동 상태, 등록된 Agent 목록(RCV/Loader) 반환, 캐시된 DataSource 설정 정보 조회 기능을 제공한다.'
        ),
        'attributes': '\n'.join([
            '@Value("${agent.zone}")',
            'private String zone;',
        ...
        'operations': '\n'.join([
            '// Agent 기동 상태 및 등록된 Agent 목록 조회',
            '@GetMapping("/health")',
            'public ResponseEntity<Map<String, Object>> health();',
            '',
            '// 캐시된 DataSource 설정 정보 조회 (디버그용)',
    ...
    pipeline_classes.append(('Controller', [{
        ...
        'overview': make_overview(
            ...
            '내부망 Agent 파이프라인 실행, 재동기화(resync), 실행 상태/결과 조회, WHERE 조건 대상 테이블 목록 조회, 파이프라인 테이블 목록 조회 기능을 제공한다.'
        ),
        'attributes': '\n'.join([
    ...
    pipeline_classes.append(('Service', [{
        ...
        'overview': make_overview(
            ...
            '내부망 파이프라인 비동기 실행을 관리한다. DataSource 해석(Proxy 경유), Connection 풀 상태 검사, Orchestrator 완료 통보, 실행 결과 캐싱 기능을 제공한다.'
        ),
        'attributes': '\n'.join([
            'private final PipelineRegistry pipelineRegistry;',
        ...
        'operations': '\n'.join([
            '// 파이프라인 비동기 실행 (agentCode 라우팅, DataSource 해석, Orchestrator 통보)',
    ...
    {
        'class_name': 'SyncDataSourceService',
        'class_type': 'Class <<Service>>',
        'overview': make_overview(
            'SyncDataSource Service를 정의한다.',
            '내부망 Agent 파이프라인 실행 시 Source/Target DataSource를 관리한다. Proxy 경유 자격증명 해석(암호문 복호화), HikariCP 커넥션 풀 생성/캐싱, 풀 상태 검사, ThreadLocal 기반 현재 DataSource 관리 기능을 제공한다.'
        ),
        'attributes': '\n'.join([
            ...
            'private static final ThreadLocal<DataSourceInfo> currentSourceDatasource;',
            'private static final ThreadLocal<DataSourceInfo> currentTargetDatasource;',
        ...
        'operations': '\n'.join([
            '// Source DataSource ID 조회 (DataSourceProvider 구현)',
            'public String getSourceDatasourceId();',
            '',
            '// Target DataSource ID 조회 (DataSourceProvider 구현)',
            'public String getTargetDatasourceId();',
            '',
            '// Agent 타입 반환 ("UNIFIED")',
            'public String getAgentType();',
            '',
            '// DB 타입 조회 (datasourceId 기준)',
            'public String getDbType(String datasourceId);',
            '',
            '// JdbcTemplate 조회 (datasourceId 기준, 미해석 시 기본 JdbcTemplate 반환)',
            'public JdbcTemplate getJdbcTemplate(String datasourceId);',
            '',
            '// Proxy 경유 DataSource 연결 정보 해석 (암호문 복호화, 캐싱)',
            'public DataSourceInfo resolveFromProxy(String datasourceId);',
            '',
            '// Connection 풀 상태 검사 (대기열/사용률/비활성 체크)',
            'public String checkPoolHealth(String datasourceId);',
            '',
            '// 현재 Source/Target DataSource 설정 (ThreadLocal)',
            'public void setCurrentDatasources(DataSourceInfo sourceDatasource, DataSourceInfo targetDatasource);',
            '',
            '// 현재 DataSource 정보 해제 (ThreadLocal)',
            'public void clearCurrentDatasources();',
            '',
            '// Source DataSource 정보 조회 (null 허용)',
            'public DataSourceInfo getSourceDatasourceInfoOrNull();',
            '',
            '// Target DataSource 정보 조회 (null 허용)',
            'public DataSourceInfo getTargetDatasourceInfoOrNull();',
            '',
            '// 캐시된 DataSource 정보 전체 조회',
            'public Map<String, DataSourceInfo> getCachedDataSourceInfos();',
            '',
            '// 모든 DataSource 종료 (애플리케이션 종료 시)',
            '@PreDestroy',
            'public void closeAll();',
        ]),
    }]))

    # --- GimsTargetRepository ---
    pipeline_classes.append(('Repository', [{
        'class_name': 'GimsTargetRepository',
        'class_type': 'Class <<Repository>>',
        'overview': make_overview(
            'GimsTarget Repository를 정의한다.',
            'GIMS Target DB JDBC 쿼리 모음. 관측소(제원) 매핑 로드, 결과 매핑 로드/자동 생성, 관측데이터 배치 INSERT, Link 테이블 배치 UPSERT 기능을 제공한다.'
        ),
        'attributes': '\n'.join([
        ...
        'operations': '\n'.join([
            ...
            '',
            '// result_id 존재 확인 및 자동 INSERT (ON CONFLICT DO NOTHING + RETURNING)',
            'public long ensureResultId(String resultTable, Map<String, Long> resultIdMap, long spotId, int iemId);',
            '',
            '// 관측데이터 배치 INSERT (EAV 확장, 1000건 단위 배치)',
            'public int batchInsertObsvdata(String tableName, List<Object[]> rows);',
            '',
            '// Link 테이블 배치 UPSERT (COALESCE로 frst_date/frst_time 보존)',
...
def main():
    ...
    run = title_p.add_run('sync-agent-bojo-int 클래스명세서')
    run.font.size = Pt(16)
    run.font.name = '맑은 고딕'
    run.bold = True
    rPr = run._r.get_or_add_rPr()
    ...
    BASE_PKG = 'kr.go.gims.agent.bojoint'

    pkg_map = {
        'HealthController': f'{BASE_PKG}.controller',
        'PipelineController': f'{BASE_PKG}.controller',
        'PipelineService': f'{BASE_PKG}.pipeline',
        'SyncDataSourceService': f'{BASE_PKG}.config',
        'GimsTargetRepository': f'{BASE_PKG}.loader.repository',
    }
    ...
    for group_name, type_groups in groups:
        ...
        for type_label, class_list in type_groups:
            ...
            for cls in class_list:
                ...
                h3 = doc.add_heading(
                    f'{group_num}.{type_num}.{cls_num}    {cls_name}', level=3)
                for run in h3.runs:
                    run.font.size = Pt(11)
                    run.font.name = '맑은 고딕'
                    rPr = run._r.get_or_add_rPr()
                ...
                package_name = pkg_map.get(cls_name, BASE_PKG)
    ...
    output_path = os.path.join(output_dir, 'sync-agent-bojo-int_NGIS_D23_클래스명세서v0.1.docx')
    doc.save(output_path)
    print(f'Generated: {output_path}')
    print(f'Total classes: 5')
