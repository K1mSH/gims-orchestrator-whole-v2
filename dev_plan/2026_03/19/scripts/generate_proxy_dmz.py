# -*- coding: utf-8 -*-
"""
sync-proxy-dmz NGIS D23 클래스명세서 생성 스크립트
- 2개 도메인 그룹, 3개 클래스
"""
...
# ============================================================
PACKAGE_MAP = {
    'Health': 'kr.go.gims.proxy.dmz.controller',
    'ConnectionInfo': 'kr.go.gims.proxy.dmz',
}
...
def build_all_classes():
    ...
    health_classes.append(('Controller', [{
        ...
        'overview': make_overview(
            'DB 프록시 전용 헬스체크 Controller를 정의한다.',
            'Proxy 앱 상태 확인(health)과 캐시된 DataSource 디버그 정보를 제공한다.'
        ),
        'attributes': '\n'.join([
            '@Value("${agent.zone}")',
            'private String zone;',
        ...
        'operations': '\n'.join([
            '// Proxy 앱 헬스체크 (status, appName, type, zone)',
            '@GetMapping("/health")',
    ...
    conninfo_classes.append(('Controller', [{
        'class_name': 'ConnectionInfoController',
        'class_type': 'Class <<Controller>>',
        'overview': make_overview(
            'Agent용 connection-info 프록시 엔드포인트 Controller를 정의한다.',
            'Agent가 파이프라인 실행 시 DB 자격증명을 얻기 위해 호출하며, Orchestrator의 connection-info API를 패스스루한다. 복호화하지 않고 암호문 그대로 전달한다.'
        ),
        'attributes': '\n'.join([
        ...
        'operations': '\n'.join([
            '// datasourceId에 해당하는 connection-info를 Orchestrator에서 조회하여 패스스루',
    ...
    conninfo_classes.append(('Service', [{
        'class_name': 'ProxyDataSourceService',
        'class_type': 'Class <<Service>>',
        'overview': make_overview(
            'DB 프록시 전용 DataSourceProvider 구현 서비스를 정의한다.',
            'Orchestrator에서 전달받은 DataSource 정보를 메모리 캐시로 관리하고, 캐시 miss 시 Orchestrator API를 호출하여 동적으로 DataSource를 생성한다. HikariCP 커넥션 풀을 사용하며, 앱 종료 시 모든 풀을 정리한다.'
        ),
        'attributes': '\n'.join([
        ...
        'operations': '\n'.join([
            '// SOURCE datasourceId 조회 (캐시에서 "source" 포함 키 탐색)',
            'public String getSourceDatasourceId();',
            '',
            '// TARGET datasourceId 조회 (캐시에서 "target" 포함 키 탐색)',
            'public String getTargetDatasourceId();',
            '',
            '// Agent 타입 반환 ("PROXY")',
            'public String getAgentType();',
            '',
            '// datasourceId에 해당하는 DB 타입 조회',
            'public String getDbType(String datasourceId);',
            '',
            '// datasourceId에 해당하는 JdbcTemplate 반환 (캐시 → Orchestrator 조회 → 기본 fallback)',
            'public JdbcTemplate getJdbcTemplate(String datasourceId);',
            '',
            '// 캐시된 DataSource 정보 맵 반환 (복사본)',
            'public Map<String, DataSourceInfo> getCachedDataSourceInfos();',
            '',
            '// 앱 종료 시 모든 HikariDataSource 커넥션 풀 정리',
            '@PreDestroy',
            'public void closeAll();',
        ]),
    }]))

    groups.append(('ConnectionInfo', conninfo_classes))
...
def main():
    ...
    # Title
    doc.add_heading('sync-proxy-dmz 클래스명세서', level=0)
    ...
    output_path = os.path.join(
        output_dir, 'sync-proxy-dmz_NGIS_D23_클래스명세서v0.1.docx'
    )
