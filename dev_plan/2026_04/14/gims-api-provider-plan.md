...
│   ├── entity/              # 엔티티
│   │   ├── ApiOperation.java       # 오퍼레이션 (API 단위)
│   │   ├── ApiOperationColumn.java # 제공 컬럼 정의
│   │   ├── ApiOperationParam.java  # WHERE 파라미터 정의
│   │   ├── ApiKeyInfo.java         # API Key 관리
│   │   └── ApiCallHistory.java     # 호출 이력
│   ├── repository/
│   ├── service/
│   │   ├── ApiProviderService.java    # 동적 쿼리 생성 + 실행
...
│   └── config/
│       └── DynamicDataSourceConfig.java  # Orchestrator 연동
├── src/main/resources/
│   └── application.yml
└── build.gradle
```
...
| 항목 | 값 |
|------|-----|
| 위치 | 내부망 (Oracle 29004 접근) |
| 포트 | 8095 (안) |
| DB | Oracle 29004 (GIMS Target) + 자체 관리 DB (PG or Oracle) |
