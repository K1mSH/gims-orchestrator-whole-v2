...
- `sync-agent-bojo` (DMZ 통합 Agent: 10 RCV + Loader + SND)
- `sync-agent-bojo-int` (Internal Agent: RCV + Loader)
- `sync-agent-others` (DMZ 기타 Agent)
- `sync-agent-provide` (Internal Provide Agent)
- 향후 추가되는 모든 파이프라인 Agent 모듈

### 적용 제외
- **API Provider** (`gims-api-provider`) — 읽기 전용 제공 서비스, Execution/SyncLog 작성 주체 아님
- **API Collector** (`infolink-api-collector`) — 별도 실행 이력 모델(`ApiExecutionHistory`) 보유. 본 invariant 범위 밖
- **Proxy** (`sync-proxy-dmz` / `sync-proxy-internal`) — Agent 가 아닌 중계 서비스. 단 관리 DB 라우팅 규약(`header-routing.md`) 의 중요 주체
...
|-------|----------------------------|-------------------------------------------|:---:|:---:|:---:|----------|-------|
| sync-agent-bojo (DMZ) | ENC 처리 (미디코드) | ? | ? | ? | ⚠️ 미확인 | - | - |
| sync-agent-bojo-int (Internal) | ENC 처리 | `internal` (추정) | ? | ✅ (4/22 확인) | ⚠️ 표 확인 필요 | - | - |
| sync-agent-others (DMZ) | ENC 처리 | ? | ? | ? | ⚠️ 미확인 | - | - |
| sync-agent-provide (Internal) | ENC 처리 (PG 29006 api_provider) | `api-provider` | ? | ✅ (4/22 헤더 라우팅 도입으로 확인) | ⚠️ 표 확인 필요 | - | - |
