---
id: VER-007
title: bojo-internal 엔티티 source_refs UK 누락
status: OPEN
created: 2026-04-23
...
bojo-internal 의 target / iftable 엔티티에 `source_refs` 컬럼은 존재(92 파일 중 일부 포함)하지만 **`@UniqueConstraint(columnNames = {"source_refs"})` 는 전무**.
...
대조 사례:
```java
// infolink-agent-bojo-dmz/entity/target/SecJewon.java:22-24
...
- VER-004 (executionId 인덱스 누락) 와 함께 bojo-internal 엔티티의 인덱스/제약 전반 재정비 필요

## 수정 범위

bojo-internal 의 source_refs 컬럼 보유 엔티티에 UK 추가:
```java
...
대상 엔티티 목록은 VER-004 와 동일 (bojo-internal/entity/target/** + iftable/**).
...
- VER-004, VER-006 (bojo-internal 엔티티 재정비 묶음)
