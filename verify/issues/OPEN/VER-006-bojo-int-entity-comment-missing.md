...
`sync-agent-bojo-int/**/entity/**/*.java` 전체에 `@Comment` / `@org.hibernate.annotations.Table(comment=...)` 미적용 (**0 건**).

다른 모든 Agent / 모듈은 엔티티에 `@Comment` 적용 상태 (bojo / others / provide / orchestrator / collector). **bojo-int 만 예외**.

## 증거

```
grep "@Comment(|@org.hibernate.annotations.Table" sync-agent-bojo-int/**/entity/**/*.java
→ 0 건
```
...
- bojo-int 관리 DB (Internal Oracle XEPDB1) 의 테이블/컬럼 **주석 부재**
...
bojo-int target 엔티티 (20+) + IF_RSV 엔티티 (14 saeol + 7 일반) 에 `@Comment` 적용:
...
- bojo-int 는 Internal Oracle 의 실 데이터 테이블 — drop 영향 큼. 대안:
