...
public class PipelineController {
    ...
    public ResponseEntity<?> execute(@RequestBody Map<String, Object> request) {
        log.info("[Bojo] 파이프라인 실행 요청: {}", request);
        ...
        try {
            ...
            log.info("[Bojo] 파이프라인 실행 수락: executionId={}, agentCode={}", executionId, agentCode);
            ...
            log.error("[Bojo] 파이프라인 실행 시작 실패", e);
    ...
    public ResponseEntity<?> getPipelineTables(@PathVariable String agentCode) {
        ...
        log.info("[Bojo] 파이프라인 테이블 목록 {}: {}", agentCode, tables);
    ...
    public ResponseEntity<List<Map<String, Object>>> getAgentInfo() {
        ...
        log.info("[Bojo] Agent 정보 조회: {} 개 Agent", result.size());
