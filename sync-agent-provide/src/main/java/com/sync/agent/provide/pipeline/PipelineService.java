...
import com.sync.agent.provide.config.pipeline.PipelineRegistry;
import com.sync.agent.provide.config.SyncDataSourceService;
...
public class PipelineService {
    ...
    public void executeAsync(String executionId, Map<String, Object> params) {
        ...
        if (agentCode == null) {
            log.error("[Provide] params 또는 executionId에서 agentCode를 찾을 수 없습니다: {}", finalExecutionId);
            return;
        }
        ...
        try {
            log.info("[Provide] 파이프라인 시작: executionId={}, agentCode={}", finalExecutionId, finalAgentCode);
            ...
            log.error("[Provide] Runner 실행 전 파이프라인 실패: {} - {}", finalExecutionId, e.getMessage(), e);
            notifyFailure(finalAgentCode, finalExecutionId, e.getMessage());
        } catch (Error err) {
            log.error("[Provide] 치명적 오류: {} - {}", finalExecutionId, err.getMessage(), err);
    ...
    private void executeWithRunner(PipelineRunner runner, OrchestratorClient orchestratorClient,
        ...
        try {
            ...
            log.info("[Provide] 파이프라인 데이터소스 설정 완료 - source: {} ({}:{}), target: {} ({}:{})",
            ...
            log.info("[Provide] 파이프라인 완료: {} 상태={}", executionId, result.getStatus());
        } catch (Exception e) {
            log.error("[Provide] 파이프라인 실패: {} - {}", executionId, e.getMessage(), e);
            ...
            log.error("[Provide] 치명적 오류: {} - {}", executionId, err.getMessage(), err);
            ...
            if (result != null) {
                try { orchestratorClient.notifyFinished(result); } catch (Exception e) {
                    log.error("[Provide] Orchestrator 알림 실패: {}", e.getMessage());
                }
            }
    ...
    private DataSourceInfo buildDataSourceInfo(Map<String, Object> params, String prefix) {
        ...
        if (portObj instanceof Integer) {
            ...
            log.error("[Provide] {} 포트가 null입니다! 사용 가능한 params: {}", prefix, params.keySet());
            port = null;
        }
    ...
    private void notifyFailure(String agentCode, String executionId, String errorMessage) {
        try {
            ...
            log.error("[Provide] Orchestrator 실패 알림 전송 실패: {}", e.getMessage());
        }
    }
